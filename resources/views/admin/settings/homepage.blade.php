        ...
        <form x-data="{ heroMediaType: '{{ old('hero_media_type', $settings['hero_media_type'] ?? 'image') }}' }" action="{{ route('admin.homepage_settings.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center space-x-6">
                            <label class="flex items-center cursor-pointer group">
                                {{-- PERUBAHAN WARNA DI SINI --}}
                                <input type="radio" name="hero_media_type" value="image" x-model="heroMediaType" class="w-4 h-4 text-red-600 border-gray-300 focus:ring-red-500">
                                <span class="ml-3 text-sm font-medium text-gray-700 group-hover:text-red-600">Gambar (Slideshow)</span>
                            </label>
                            <label class="flex items-center cursor-pointer group">
                                {{-- PERUBAHAN WARNA DI SINI --}}
                                <input type="radio" name="hero_media_type" value="video" x-model="heroMediaType" class="w-4 h-4 text-red-600 border-gray-300 focus:ring-red-500">
                                <span class="ml-3 text-sm font-medium text-gray-700 group-hover:text-red-600">Video</span>
                            </label>
                        </div>
                    </div>
                    ...
                    {{-- PERUBAHAN WARNA DI SINI --}}
                    <div x-show="heroMediaType === 'video'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" class="bg-gradient-to-br from-red-50 to-orange-50 border-2 border-dashed border-red-300 rounded-lg p-6">
                        ...
                        <div class="mt-4">
                            <input type="file" name="hero_video" accept="video/mp4,video/webm" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-red-50 file:text-red-700 hover:file:bg-red-100">
                                ...
                                <div class="bg-gradient-to-br from-gray-50 to-gray-100 border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow duration-200">
                                    <div class="flex items-center justify-between mb-3">
                                        ...
                                        <div class="w-8 h-8 bg-red-100 rounded-full flex items-center justify-center">
                                            <span class="text-red-600 font-bold text-sm">{{ $i }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-3">
                                        <div>
                                            ...
                                            {{-- PERUBAHAN WARNA DI SINI --}}
                                            <input type="file" name="hero_slide_{{ $i }}_image" class="block w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-medium file:bg-red-50 file:text-red-700 hover:file:bg-red-100">
                                            @if(!empty($settings['hero_slide_' . $i . '_image']))
                                                <div class="mt-2 relative">
                                                    ...
                                                    <div class="absolute top-1 right-1 bg-green-500 text-white text-xs px-1 py-0.5 rounded">✓</div>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="space-y-2">
                                ...
                                <input type="url" name="social_link_facebook" value="{{ $settings['social_link_facebook'] ?? '' }}" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 text-sm">
                            </div>
                            <div class="space-y-2">
                                ...
                                <input type="url" name="social_link_youtube" value="{{ $settings['social_link_youtube'] ?? '' }}" class="block w-full border-gray-100 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 text-sm">
                            </div>
                            <div class="space-y-2">
                                <label for="social_link_instagram" class="block text-sm font-medium text-gray-700 flex items-center">
                                    <svg class="w-4 h-4 mr-2 text-pink-600" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 2C7.794 2 7.556 2.01 6.648 2.048...z" clip-rule="evenodd" />
                                    </svg>
                                    Instagram URL
                                </label>
                                <input type="url" name="social_link_instagram" value="{{ $settings['social_link_instagram'] ?? '' }}" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 text-sm">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex justify-end">
                {{-- PERUBAHAN WARNA DI SINI --}}
                <button type="submit" class="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-semibold py-3 px-8 rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-200 flex items-center">
