    ...
    <div class="bg-white p-6 md:p-8 rounded-lg shadow-lg">
        <form action="{{ route('admin.program-studi.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="accreditation" id="accreditation" value="{{ old('accreditation') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
            </div>

            <div>
                <label for="link" class="block text-sm font-medium text-gray-700">Link Halaman Detail (URL)</label>
                <input type="url" name="link" id="link" value="{{ old('link') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" placeholder="https://...">
            </div>
