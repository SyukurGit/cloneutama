...
<div class="container mx-auto" x-data="{
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        ...
        <a href="{{ route('admin.berita.create') }}" class="w-full sm:w-auto bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg shadow-md flex-shrink-0 flex items-center justify-center">
        ...
        <form action="{{ route('admin.berita.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                ...
                <div class="md:col-span-2">
                    <div class="relative">
                        <input type="search" name="search" value="{{ $search ?? '' }}" placeholder="Cari judul berita..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500">
                ...
                <div class="flex items-center gap-2">
                    <div class="relative flex-grow" x-on:click.away="tagDropdownOpen = false">
                            ...
                            <div class="space-y-2 max-h-60 overflow-y-auto">
                                @foreach($tags as $tag)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, $selectedTags ?? []) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-red-600 focus:ring-red-500">
                ...
                <div class="flex items-center justify-end gap-2">
                    <button @click="layout = 'grid'; localStorage.setItem('news_layout', 'grid')" type="button" :class="{ 'bg-red-600 text-white': layout === 'grid', 'bg-gray-200 text-gray-600': layout !== 'grid' }" class="p-2 rounded-lg transition-colors" title="Tampilan Grid">
                        <i class="fas fa-th-large"></i>
                    </button>
                    <button @click="layout = 'list'; localStorage.setItem('news_layout', 'list')" type="button" :class="{ 'bg-red-600 text-white': layout === 'list', 'bg-gray-200 text-gray-600': layout !== 'list' }" class="p-2 rounded-lg transition-colors" title="Tampilan List">
                ...
                <div class="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col hover:shadow-xl transition-shadow duration-300">
                    ...
                    <div class="p-4 flex flex-col flex-grow">
                        <div class="flex flex-wrap gap-2 mb-2">
                            @foreach($news->tags->take(2) as $tag)
                                <span class="bg-red-100 text-red-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">{{ $tag->name_en }}</span>
                            @endforeach
                        </div>
                    ...
                    <div class="p-4 bg-gray-50 border-t flex justify-between items-center">
                        ...
                        <div class="flex gap-4">
                            ...
                            <form action="{{ route('admin.berita.destroy', $news) }}" method="POST" onsubmit="return confirm('Anda yakin ingin memindahkan berita ini ke tong sampah?');">
                                @csrf
                                @method('DELETE')
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($newsItems as $news)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ optional($news->published_at)->format('d M Y') ?? 'N/A' }}</td>
